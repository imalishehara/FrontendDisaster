...
export default function OngoingAlerts() {
  ...
  useEffect(() => {
    ...
    const parsed = JSON.parse(storedData);
    const division = parsed.dsDivision || parsed.DsDivision;

    if (!division) {
      alert("DS Division not found in stored data.");
      return;
    }

    setDsDivision(division);
    fetchOngoingAlertsByDivision(division);
  }, []);
    ...
    <div className="w-full max-w-4xl mx-auto bg-gray-100 rounded-2xl shadow p-8 mt-8">
      ...
      <div className="overflow-x-auto">
        {loading ? (
          ...
          <p className="text-center py-4 text-red-600">
            No ongoing alerts found for this DS division.
          </p>
        ) : (
          <table className="min-w-full bg-white rounded-lg">
            <thead>
